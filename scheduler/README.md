...
```go
import "github.com/Tsukikage7/microservice-kit/scheduler"
```
...
import (
    ...
    "github.com/Tsukikage7/microservice-kit/scheduler"
)

func main() {
    log, _ := logger.NewLogger(&logger.Config{Level: "debug"})

    // 创建调度器
    s := scheduler.MustNew(
        scheduler.WithLogger(log),
        scheduler.WithDefaultTimeout(10*time.Minute),
    )
    ...
    s.Add(scheduler.NewJob("sync-data").
        ...
        Singleton().                 // 防止重叠执行
        Retry(3, time.Second*10).    // 失败重试 3 次
        MustBuild(),
    )
