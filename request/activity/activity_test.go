...
func TestHTTPMiddleware(t *testing.T) {
	...
	// 检查是否记录了活跃
	status, _ := tracker.GetStatus(context.Background(), "test-user")
	if status == nil {
...
func TestConsumer(t *testing.T) {
	...
	status, _ := tracker.GetStatus(context.Background(), "user456")
	if status == nil {
...
func TestBatchHandler(t *testing.T) {
	...
	err := handler.HandleBatch(context.Background(), messages)
	if err != nil {
		t.Fatalf("HandleBatch() error = %v", err)
	}

	// 应该只有最新的一条
	status, _ := tracker.GetStatus(context.Background(), "user789")
	if status == nil {
